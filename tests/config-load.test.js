/**
 * Loading a config into a **form-shaped** script: which field takes which value from the file, and
 * the `@fromFile:` annotation that says so surviving a trip through the form serializer.
 *
 * The import button these were written for is gone — auto-import replaced it, and it fills a config
 * *block* rather than a form. `applyFileConfig` and `hasFileFields` are the per-field spelling, which
 * no shipped script uses yet; they are kept and tested because they are the path a form-shaped script
 * will need, and DEFERRED.md records that they have no caller today.
 *
 * There is no precedence ladder here and no dirty tracking, on purpose.
 *
