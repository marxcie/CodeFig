...
| `npm run figma:sync` | Blocks until the open plugin reports the build id in `dist/build-id.txt`. **Figma cannot reload a plugin from outside it** — there is no self-relaunch API, and `figma_reload_plugin` in the MCP tooling refreshes a different plugin's iframe in a different file (both tried). So the reload click stays; this removes the conversation around it. Run it after every dev build and before verifying anything. |
| `npm run figma:ui -- <command>` | Drives the **plugin UI** from the terminal: `listScripts`, `selectScript`, `readConfig`, `writeConfig`, `readInfoPanel`, `readPreview`, `readTabs`, `pressImport`, `readButtonState`. Needs `npm run dev` and the plugin open on a dev build. Dev only, and **refuses** to print an answer from a stale build rather than warning and continuing — a result from the wrong build looks like a pass, which is how two things got "verified" against code no longer on disk. `--allow-stale` to override. |
