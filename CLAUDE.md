...
| `npm run figma:ui -- <command>` | Drives the **plugin UI** from the terminal: `listScripts`, `selectScript`, `readConfig`, `writeConfig`, `readInfoPanel`, `readPreview`, `readTabs`, `pressImport`, `readButtonState`. Needs `npm run dev` and the plugin open on a dev build. Dev only, and the same build-id staleness check `figma:run` does. |
